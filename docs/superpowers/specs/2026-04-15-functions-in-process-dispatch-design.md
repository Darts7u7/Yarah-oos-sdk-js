...
The trigger is the presence of `globalThis.__yarah_dispatch__`, which the auto-generated router publishes at module load. No new SDK config flag is required; existing `isServerMode` keeps its current CSRF/localStorage semantics and is not involved.
...
**Location:** the `generateRouter(functions)` function in the Yarah backend (separate repo from this SDK; the function body is reproduced in the original brainstorming context).
...
async invoke<T>(slug: string, options: FunctionInvokeOptions = {}) {
  ...
  if (typeof dispatch === 'function') {
    try {
      ...
      return {
        data: null,
        error: error instanceof YarahError
          ? error
          : new YarahError(
...
- **URL:** `new URL('/' + slug, 'http://yarah.local').toString()`. The router only reads `pathname`; the placeholder host/scheme are intentionally non-routable to make the in-process intent explicit. Slugs containing `/` (e.g. `'foo/bar'`) become pathname `/foo/bar` and exercise the router's existing subpath rewrite.
...
- Non-2xx → throw `YarahError`, preferring `YarahError.fromApiError(data)` when the body has the `{ error, ... }` shape, otherwise a generic `YarahError(statusText, status, 'REQUEST_FAILED')`.
- Body parse failures → `YarahError('Failed to parse response body…', status, response.ok ? 'PARSE_ERROR' : 'REQUEST_FAILED')`.
...
This lets `globalThis.__yarah_dispatch__` be referenced directly without `as any` casts. The `export {}` keeps the file a module so `declare global` works.
...
| `dispatch` returns non-2xx with `{ error, message }` body | `{ data: null, error: YarahError(status, code) }` (preserves all fields) |
| `dispatch` returns non-2xx with non-error body            | `{ data: null, error: YarahError(status, 'REQUEST_FAILED') }`            |
| `dispatch` returns 404 (slug not in routes)               | Returned as error (no HTTP fallback)                                        |
| `dispatch` throws synchronously or rejects                | `{ data: null, error: YarahError(500, 'FUNCTION_ERROR') }`               |
...
New file `src/modules/__tests__/functions.test.ts`. Each test sets/clears `globalThis.__yarah_dispatch__` in setup/teardown. Mock `dispatch` with `jest.fn()` returning crafted `Response` objects.
...
| 5   | global present, dispatch throws `new Error('boom')`                                                         | Returns `{ data: null, error: YarahError('boom', 500, 'FUNCTION_ERROR') }`                               |
